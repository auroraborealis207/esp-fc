...
Device::InputDevice * Input::getInputDevice()
{
  ...
  else if(_model.isFeatureActive(FEATURE_RX_PPM) && _model.config.pin[PIN_INPUT_RX] != -1)
  {
    ...
    return &_ppm;
  }else if(_model.isFeatureActive(FEATURE_RX_PWM) && _model.config.pin[PIN_INPUT_RX] != -1)
  {
    uint8_t pin[8] = {_model.config.pin[PIN_INPUT_0],
                      _model.config.pin[PIN_INPUT_1],
                      _model.config.pin[PIN_INPUT_2],
                      _model.config.pin[PIN_INPUT_3],
                      _model.config.pin[PIN_INPUT_4],
                      _model.config.pin[PIN_INPUT_5],
                      _model.config.pin[PIN_INPUT_6],
                      _model.config.pin[PIN_INPUT_7],
                      };
    
    _pwm.begin(pin, _model.config.input.pwmMode);
    _model.logger.info().logln(F("RX_PWM"))
    .log(F("Pin 1 "))
    .log(_model.config.pin[PIN_INPUT_0])
    .log(F("Pin 2 "))
    .log(_model.config.pin[PIN_INPUT_1])
    .log(F("Pin 3 "))
    .log(_model.config.pin[PIN_INPUT_2])
    .log(F("Pin 4 "))
    .log(_model.config.pin[PIN_INPUT_3])
    .log(F("Pin 5 "))
    .log(_model.config.pin[PIN_INPUT_4])
    .log(F("Pin 6 "))
    .log(_model.config.pin[PIN_INPUT_5])
    .log(F("Pin 7 "))
    .log(_model.config.pin[PIN_INPUT_6])
    .log(F("Pin 8 "))
    .log(_model.config.pin[PIN_INPUT_7])
    .logln(_model.config.input.pwmMode);
    return &_pwm;
  }
